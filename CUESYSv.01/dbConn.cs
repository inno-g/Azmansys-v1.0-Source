...
namespace CUESYSv._01
{
    public class dbConn
    {
        ...
        public void insertBooking(string custContact, string flyingFrom, string Destination, string bookingDate, string bookingTime, string flightPrice, string bookingPaid)
        {
            MySqlCommand comm = conn.CreateCommand();
            comm.CommandText = "INSERT INTO `tblBookings` (`custContact`, `flyingFrom`, `Destination`, `bookingDate`, `bookingTime`, `flightPrice`, `bookingPaid`) VALUES (@custContact, @flyingFrom, @Destination, @bookingDate, @bookingTime, @flightPrice, @bookingPaid);";
            comm.Parameters.AddWithValue("@custContact", custContact);
            comm.Parameters.AddWithValue("@flyingFrom", flyingFrom);
            ...
            comm.Parameters.AddWithValue("@flightPrice", flightPrice);
